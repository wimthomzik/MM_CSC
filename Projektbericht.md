...
2. **Funktion `readDynamicLine`**: Diese Funktion liest eine Zeile aus einer Datei und speichert die Werte in einem dynamisch allozierten Vektor. Sie ignoriert leere Zeilen sowie überflüssige Kommata und Leerzeichen. Die Funktion nimmt als Parameter einen Dateipointer, einen pointer auf den Vektor, einen pointer auf die Größe des Vektors und den Datentyp des Vektors entgegen.

   - **Deklaration und Initialisierung von Variablen**: pointer auf die zu lesende Zeile, deren Größe und die Anzahl der gelesenen Zeichen werden initialisiert.
...
3. **Funktion `readCSCMatrix`**: Diese Funktion liest eine CSC-Matrix aus einer Textdatei. Sie öffnet die Datei, liest die Dimensionen der Matrix, die Datenwerte, die Zeilenindizes und die Spaltenpointer. Anschließend überprüft sie, ob die Anzahl der Zeilenindizes mit der Anzahl der Nicht-Null-Elemente übereinstimmt und ob keine zusätzlichen Daten nach den erwarteten Matrizenwerten vorhanden sind.
   ...
   - **Lesen der Spaltenpointer**: Die Funktion `readDynamicLine` wird aufgerufen, um die Spaltenpointer der Matrix zu lesen. Falls ein Fehler auftritt, wird eine Fehlermeldung ausgegeben und die Datei geschlossen.
   ...
   - **Überprüfen der Spaltenpointer**: Die Funktion überprüft, ob die Anzahl der Spaltenpointer mit der Anzahl der Spalten übereinstimmt. Falls nicht, wird eine Fehlermeldung ausgegeben und die Datei geschlossen.
...
4. **Fehlerbehandlung**
   ...
   - **Dateioperationen**: Falls das Öffnen der Datei fehlschlägt, wird eine Fehlermeldung ausgegeben. Falls das Lesen der Dimensionen, Datenwerte, Zeilenindizes oder Spaltenpointer fehlschlägt, wird eine Fehlermeldung ausgegeben und die Datei wird geschlossen.
   - **Datenkonsistenz**: Falls die Anzahl der Zeilenindizes nicht mit der Anzahl der Nicht-Null-Elemente übereinstimmt, wird eine Fehlermeldung ausgegeben und die Datei wird geschlossen. Falls nach den erwarteten Matrizenwerten zusätzliche Daten vorhanden sind, wird eine Fehlermeldung ausgegeben und die Datei wird geschlossen. Falls die Anzahl der Spaltenpointer nicht mit der Anzahl der Spalten übereinstimmt, wird eine Fehlermeldung ausgegeben und die Datei wird geschlossen.
   - **Allokation von null Zeilen in einer Null-Matrix**: Wenn die Anzahl der Nicht-Null-Elemente null ist, stellt die Funktion sicher, dass alle Einträge in den Spaltenpointern ebenfalls null sind. Falls dies nicht der Fall ist, wird eine Fehlermeldung ausgegeben und die Datei wird geschlossen.

#### csc_matrix_writer.c

Die Datei `csc_matrix_writer.c` enthält Funktionen zum Schreiben von Matrizen im CSC-Format in Textdateien.

1. **Funktion `writeCSCMatrix`**: Diese Funktion schreibt eine CSC-Matrix in eine Textdatei. Sie nimmt als Parameter den Dateinamen und einen pointer auf die Matrixstruktur entgegen.

   - **Überprüfung der Eingabeparameter**: Die Funktion überprüft, ob der Dateiname und der Matrixpointer gültig sind. Falls nicht, wird eine Fehlermeldung ausgegeben und `EXIT_FAILURE` zurückgegeben.
   ...
   - **Schreiben der Spaltenpointer**: Die Spaltenpointer der Matrix werden in die Datei geschrieben. Jeder pointer wird durch ein Komma getrennt. Falls dies fehlschlägt, wird eine Fehlermeldung ausgegeben und die Datei geschlossen.
...
2. **Fehlerbehandlung**: Die Funktion enthält umfangreiche Fehlerbehandlung, um sicherzustellen, dass die Matrixdaten korrekt in die Datei geschrieben werden. Im Falle eines Fehlers gibt die Funktion entsprechende Fehlermeldungen aus und sorgt dafür, dass die Datei ordnungsgemäß geschlossen wird.
   ...
   - **Überprüfung der Eingabeparameter**: Falls der Dateiname oder der Matrixpointer ungültig ist, wird eine Fehlermeldung ausgegeben.
   ...
   - **Fehler beim Schreiben der Spaltenpointer**: Falls das Schreiben der Spaltenpointer fehlschlägt, wird eine Fehlermeldung ausgegeben und die Datei geschlossen.
...
#### csc_matrix_mult_V0.c
   ...
   - **Initialisierung der Ergebnismatrix**: Die Funktion initialisiert die Dimensionen und speichert für die Ergebnismatrix. Der worst-case Speicherbedarf wird abgeschätzt und entsprechend Speicher für die Werte, Zeilenindizes und Spaltenpointer allokiert. Falls die Allokation fehlschlägt, wird eine Fehlermeldung ausgegeben und das Programm beendet.

   - **Initialisierung der Spaltenpointer**: Die Spaltenpointer der Ergebnismatrix werden auf 0 gesetzt.
   ...
   - **Aktualisierung der Spaltenpointer und Nicht-Null-Elemente**: Nach der Verarbeitung jeder Spalte wird der Spaltenpointer der Ergebnismatrix aktualisiert. Nach Abschluss der Multiplikation wird die Anzahl der Nicht-Null-Elemente in der Ergebnismatrix gespeichert.
...
- **Multiplikation der Matrizen**: Die Funktion iteriert über jede Spalte der zweiten Matrix und für jede Spalte über jeden Wert in dieser Spalte. Für jeden Wert wird die entsprechende Spalte der ersten Matrix durchsucht, um die Multiplikation durchzuführen und das Ergebnis in der Ergebnismatrix zu speichern.
   ...
   - **Verwendung eines Buffers**: Ein statischer Buffer wird verwendet, um die Zeilenindizes zu speichern, die bereits einen Wert in der Ergebnismatrix haben. Dies verhindert das Backtracking durch die bereits eingetragenen Werte einer Spalte.
...
- **Multiplikation der Matrizen**: Die Funktion iteriert über jede Spalte der zweiten Matrix und für jede Spalte über jeden Wert in dieser Spalte. Für jeden Wert wird die entsprechende Spalte der ersten Matrix durchsucht, um die Multiplikation durchzuführen und das Ergebnis in der Ergebnismatrix zu speichern.
   ...
   - **SIMD-Optimierung**: Die Funktion verwendet SIMD-Instruktionen, um die Multiplikationen zu beschleunigen. Die Werte der ersten Matrix werden in 4er-Blöcken geladen und mit einem Skalar multipliziert, der aus der zweiten Matrix stammt.
...
Um die Leistung und Effizienz der Matrizenmultiplikation im CSC-Format zu verbessern, wurden mehrere Optimierungen vorgenommen. Diese Optimierungen umfassen u.A. die Verwendung von Buffern zur Vermeidung von Backtracking und die Implementierung von SIMD (Single Instruction, Multiple Data) Intrinsics zur Beschleunigung von Berechnungen. Jede dieser Techniken zielt darauf ab, die Rechenzeit zu verkürzen und die Speichernutzung zu optimieren.
...
- **Implementierung**: Der Buffer speichert für jede Zeile, ob bereits ein Wert in der aktuellen Spalte der Ergebnismatrix vorhanden ist. Wenn ein neuer Wert berechnet wird, wird zuerst der Buffer überprüft. Wenn dort bereits ein Wert existiert, wird der neue Wert zum vorhandenen addiert. Andernfalls wird der neue Wert eingefügt und der Buffer entsprechend aktualisiert.
...
- **Grund**: SIMD ermöglicht die gleichzeitige Ausführung derselben Operation auf mehreren Datenpunkten, was die Berechnungszeit deutlich verkürzt.
- **Implementierung**: Die Werte der ersten Matrix werden in 4er-Blöcken geladen und mit einem Skalar aus der zweiten Matrix multipliziert. Diese Vektorisierung der Berechnungen reduziert die Anzahl der notwendigen Schleifendurchläufe und nutzt die Prozessorarchitektur effizienter aus. Die SIMD-Instruktionen werden durch Intrinsics wie `_mm_load_ps` und `_mm_mul_ps` implementiert.
...
- **Optimierung der Speicherverwaltung**: Die dynamische Speicherallokation könnte implementiert werden, um hohen Speicherverbrauch zu vermeiden. Dies ist aber nicht in allen Formen und Anwendungen eine Optimierung und kann sogar zu Laufzeitverschlechterungen führen.
- **Unterstützung weiterer Formate**: Die Implementierung könnte erweitert werden, um zusätzliche dünnbesetzte Matrixformate wie z.B. CSR (Compressed Sparse Row Format) zu unterstützen. Dies würde die Flexibilität und Anwendbarkeit des Programms erhöhen.
